...
public class ViewIncomingVehicleDetails extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
            ...
            Date now = new Date();
            String strDate = ff.format(now);
            System.out.println("OUTTIME : "+strDate); 
			Connection connection = DatabaseConnection.getConnection();
			Statement statement = connection.createStatement();
			String query="update  tblvehicle set OutTime='"+strDate+"',Remark='" + remark + "',Status='" + status
                    + "',ParkingCharge='" + parkingcharge + "' where ID='" + session.getAttribute("cid") + "' and vehiclecategory='"+category+"'";
			System.out.println("OUT QUERY :"+query);
			int updateinfo = statement.executeUpdate(query);
			if (updateinfo > 0) {
